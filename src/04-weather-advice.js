...
export function getWeatherAdvice(temperature, isRaining) {
  if(!isRaining){
    ...
    else if(temperature >= 25 && temperature <= 34) return "Great weather for hiking - don't forget sunscreen"
    else if(temperature >= 15 && temperature <= 24) return "Perfect hiking weather - enjoy the trails"
    else if(temperature >= 5 && temperature <= 14) return "Chilly - wear layers for your hike"
    else return "Too cold - stay warm indoors"
  }
  else {
    if (temperature >= 35) return "Too hot for hiking - stay indoors and hydrate"
    else if (temperature >= 25) return "Warm but rainy - consider indoor activities"
    else if (temperature >= 15 && temperature <= 24) return "Cool and rainy - bring waterproof gear if hiking"
    else if (temperature >= 5 && temperature <= 14)return "Cold and wet - best to stay indoors"
    else return "Too cold - stay warm indoors"
  }
}
